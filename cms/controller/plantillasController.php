...
class plantillasController extends Controller
{
  ...
  {
  	$data['title'] = 'Listando Plantillas';
  	$data['tabla'] = $this->_tabla;
  	$data['type'] = $type;
    
    $this->getBreadcrumb('Plantillas','listar');

		if(Validator::validateInteger('listar', true) == 1) {
	  	$opt = Validator::sanitizeInteger('accion', true);
	  	$ids = $_POST['selected'];	  	
	  	
	  	($opt == 4) ? $this->_model->deletePlantillas($ids) : $this->_model->updatePlantillas($opt,$ids);				
	  }
	  	   
		$data['base_url'] = $this->base_url;
		$data['heads'][] = array('title'=>'Nombre','sort'=>'sisort','align'=>'left');
		$data['heads'][] = array('title'=>'Archivo','sort'=>'sisort','align'=>'left');
  ...
  public function insertar()
  {
    ...
    if(Validator::validateInteger('actualizar', true) == 1) {
      ...
      $nombre = Validator::sanitizeString($fields['Nombre']);
		  ...
		  if(empty($nombre)) { 
		  	$data['alert'] = $this->alertMessage('danger','Debe ingresar el Nombre de la Plantilla');
		  	$this->_view->assign('data',$data);    
    		$this->_view->render();
		    exit;
		  }  		 
		  $id = $this->_model->updatePlantilla($fields);
  ...
  public function editar($id)
  {
    ...
    if(!$this->_model->getPlantilla($id)) Url::redirect();
    ...
    if(Validator::validateInteger('actualizar', true) == 1) {
      ...
      $nombre = Validator::sanitizeString($fields['Nombre']);
	    ...
	    $fields['ID'] = $id;
		  $this->_model->updatePlantilla($fields);

      Url::redirect($this->base_url.'listar/1');
    }
    
    $data[$this->_tabla] = $this->_model->getPlantilla($id);  	
  	$this->_view->assign('data',$data);    
 		$this->_view->render();
  }
  ...
  public function filter() 
	{		
		$cols = array('ID','Nombre','Archivo','Publico');   
