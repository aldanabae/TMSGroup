...
class partnersController extends Controller {
  ...
  public function listar($type = 1) {
    $data['title'] = 'Listando Partners';
    $data['tabla'] = $this->_tabla;
    $data['type'] = $type;

    $this->getBreadcrumb('Partners', 'listar');

    if (Validator::validateInteger('listar', true) == 1) {
      $opt = Validator::sanitizeInteger('accion', true);
      $ids = $_POST['selected'];

      ($opt == 4) ? $this->_model->deletePartners($ids) : $this->_model->updatePartners($opt, $ids);
    }
  ...
  public function insertar() {
    ...
    if (Validator::validateInteger('actualizar', true) == 1) {
      ...
      if (empty($titulo)) {
        $data['alert'] = $this->alertMessage('danger', 'Debe ingresar el Nombre del Partner');
        $this->_view->assign('data', $data);
        $this->_view->render();
        exit;
      }

      $id = $this->_model->updatePartner($fields);
  ...
  public function editar($id) {
    ...
    if (Validator::validateInteger('actualizar', true) == 1) {
      ...
      $fields['ID'] = $id;
      $this->_model->updatePartner($fields);

      Url::redirect($this->base_url . 'listar');
    }

    $data[$this->_tabla] = $this->_model->getPartner($id);
