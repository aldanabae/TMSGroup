...
class sys_tablesController extends Controller {
  ...
  public function listar() {
    $data['title'] = 'Listando Tablas BD';
    $data['tabla'] = $this->_tabla;

    $this->getBreadcrumb('Tablas BD', 'listar');

    $data['base_url'] = $this->base_url;
    $data['heads'][] = array('title' => 'Nombre', 'sort' => 'sisort', 'align' => 'left');
    $data['heads'][] = array('title' => 'T&iacute;tulo', 'sort' => 'sisort', 'align' => 'left');
  ...
  public function insertar() {
    ...
    if (Validator::validateInteger('actualizar', true) == 1) {
      ...
      if (empty($nombre)) {
        $data['alert'] = $this->alertMessage('danger', 'Debe ingresar el Nombre de la Tabla BD');
        $this->_view->assign('data', $data);
        $this->_view->render();
        exit;
      }
      $id = $this->_model->updateTable($fields);
  ...
  public function editar($id) {
    ...
    if (Validator::validateInteger('actualizar', true) == 1) {
      ...
      $fields['ID'] = $id;
      $this->_model->updateTable($fields);
      Url::redirect($this->base_url . 'listar');
    }
    $data[$this->_tabla] = $this->_model->getTable($id);
  ...
  public function filter() {
    $cols = array('ID', 'Nombre', 'Titulo', 'Publico');
