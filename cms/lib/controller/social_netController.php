...
class social_netController extends Controller {
  ...
  public function listar($type = 1) {
    $data['title'] = 'Listando Redes Sociales';
    $data['tabla'] = $this->_tabla;
    $data['type'] = $type;

    $this->getBreadcrumb('Redes', 'listar');

    if (Validator::validateInteger('listar', true) == 1) {
      $opt = Validator::sanitizeInteger('accion', true);
      $ids = $_POST['selected'];
      ($opt == 4) ? $this->_model->deleteRedes($ids) : $this->_model->updateRedes($opt, $ids);
    }
  ...
  public function insertar() {
    ...
    if (Validator::validateInteger('actualizar', true) == 1) {
      ...
      if (empty($titulo)) {
        $data['alert'] = $this->alertMessage('danger', 'Debe ingresar el T&iacute;tulo de la Red Social');
        $this->_view->assign('data', $data);
        $this->_view->render();
        exit;
      }

      $id = $this->_model->updateRed($fields);
  ...
  public function editar($id) {
    ...
    if (Validator::validateInteger('actualizar', true) == 1) {
      ...
      $fields['ID'] = $id;
      $this->_model->updateRed($fields);
      Url::redirect($this->base_url . 'listar/1');
    }

    $data[$this->_tabla] = $this->_model->getRed($id);
    $this->_view->assign('data', $data);
    $this->_view->render();
  }
