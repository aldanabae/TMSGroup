  ...
  $(document.body).dropzone({
    ...
    success: function (file, data) {
      ...
      if (isJson && json.success == 1) {
        ...
        count = $('#box-gallery .gal-item').size();
...
function getTableList(aTableName, asInitVals, aUrlFilter, aUrlEdit) {
  var aUrlEdit = aUrlEdit;
  var oTable = $('#' + aTableName).dataTable({
    'iDisplayLength': 25,
    'bProcessing': true,
    'bServerSide': true,
    'bAutoWidth': false,
  ...
  $('thead input').keyup(function () {
    //sumamos 1 porque la primera columna no se cuenta
